...
impl LutIndex {
    ...
    pub fn insert_token_seeded(&mut self, token: Token, seed: u64) {
        ...
        self.insert_token_at(token, bit);
    }

    /// Incremental insertion at an explicit bit address: register `token` in
    /// the fiber of `bit`.
    ///
    /// This is the low-level splice used by n-gram ingestion, where the
    /// sketch atom is the **n-gram's** address but the fiber stores the token
    /// the n-gram points at.
    pub fn insert_token_at(&mut self, token: Token, bit: u32) {
...
mod tests {
    ...
    #[test]
    fn insert_token_at_registers_an_explicit_fiber() {
        let mut idx = LutIndex::default();
        // The n-gram "a\0b" hashes to some bit; the fiber at that bit holds
        // the *pointed-to* token ("a"), not the n-gram bytes.
        let ngram = b"a\0b".to_vec();
        let bit = BitAddress::of_token(&ngram).bit();
        idx.insert_token_at(b"a".to_vec(), bit);
        assert!(idx.fiber(bit).contains(&b"a".to_vec()));
        assert!(!idx.fiber(bit).contains(&ngram));
    }
